...
class selectionStatsController {
    async patchRecall(req: Request, res: Response){
        ...
        const result: Query<RecallDTO> = await collectionRecalls.findOneAndUpdate({count: recall}, { $set: {count: recall + 1}});
    ...
    async patchLike(req: Request, res: Response){
        ...
        const result: Query<LikeDTO> = await collectionSelectionStats.findOneAndUpdate({likes: selectionStats}, { $set: {likes: selectionStats + 1}});
    ...
    async patchDislike(req: Request, res: Response){
        ...
        const result: Query<DislikeDTO> = await collectionSelectionStats.findOneAndUpdate({dislikes: selectionStats}, { $set: {dislikes: selectionStats + 1}});
