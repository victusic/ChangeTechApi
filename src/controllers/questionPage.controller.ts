...
import { AnswerDTO, AnswerResultDTO, Query, QuestionDTO, VectorElDTO } from "../types";
...
class questionPageController {
    ...
    async getAnswers(req: Request, res: Response){
        ...
        switch (region) {
            case 'RU':
                answers = await db.query('SELECT id, type, "textRu" AS "text", "rangeMin", "rangeMax" FROM "Anser" WHERE id IN (SELECT anser FROM "Survey" WHERE question=$1)', [questionId]);
                break;
            case 'KZ':
                answers = await db.query('SELECT id, type, "textKz" AS "text", "rangeMin", "rangeMax" FROM "Anser" WHERE id IN (SELECT anser FROM "Survey" WHERE question=$1)', [questionId]);
                break;
            default:
                answers = await db.query('SELECT id, type, "textEng" AS "text", "rangeMin", "rangeMax" FROM "Anser" WHERE id IN (SELECT anser FROM "Survey" WHERE question=$1)', [questionId]);
                break;
        }
    ...
    async getVector(req: Request, res: Response){
        ...
        const vector: Query<VectorElDTO[]> = await db.query('SELECT id, spoiling FROM "VectorParameters" WHERE category = $1', [category]);
    ...
    async getAnswerResult(req: Request, res: Response){
        ...
        const result: Query<AnswerResultDTO[]> = await db.query('SELECT "vectorParameter", value FROM "Survey" WHERE anser = $1', [answer]);
